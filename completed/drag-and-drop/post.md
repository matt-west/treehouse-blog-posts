...
To get use started we are first going to take a look at how to make HTML element draggable. This is done using the `draggable` attribute.
...
* `drag` - This event is continuously fired on element being dragged during the interaction.
...
* `dragover` - This event is continuously fired whilst the dragged element is in the target area.
* `drop` - This event is fired when the element or file is dropped.
* `dragend` - This event is fired once the drag interaction has finished. It applies to the element that was dragged.
...
When a drag interaction begins a `DataTransfer` object is created that is associated with the interaction. This object is used to store information about the drag interaction as well as data items.

Lets take a look at some of the properties and methods that the `DataTransfer` object has.

* `dropEffect` - The type of drag and drop interaction. This determines what cursor the browser should display during the interaction. Possibly values are: copy, move, link and none.
...
* `setDragImage(imgElement, x, y)` - This method is used to specify a custom image that should be displayed when the element is being dragged. By default many browsers will just display a semi-transparent version of the source element. You should pass in an `img` element as well as `x` and `y` that specify the position of the image relative to the mouse cursor.

The `dataTransfer` object is accessible on the event that is passed in to your event listener function blocks. For example:
...
Now that the HTML is setup we need to switch over to a JavaScript file and start writing out code to handle the Drag and Drop interaction. Lets start by create some variables and initializing them with the elements in our markup.

The `elementDragged` variable will be used to track which of the elements is currently being dragged.
...
When the `dragstart` event is fired we need to set the value of the `effectAllowed` property to `move` as we are moving the element into the drop target. We are also going to store the text content of the dragged element with our `DataTransfer` object. Finally we'll set the `elementDragged` variable that we created earlier to the element that is being dragged.
...
It would be nice if the styling of the drop zone changed when the element was dragged over it. To do this we need to apply a class to the drop target when the `dragenter` event is fired and then remove the class when `dragleave` is fired.

***
**Note**: We use `dragenter` and `dragleave` rather than `dragover` because `dragover` is called continuously whilst an element is over the target whilst `dragenter` and `dragleave` are only called once each. It's best to minimize the amount of work the browser has to do wherever possible.
***
...
Now that you're feeling more confident with the Drag and Drop API lets take a look at an example that is a bit more complex.

In this section you are going to learn how to drag a text file into the browser, read the contents of the file and the display this content on the page. 
...
***
**Note**: There is no `dragstart` event fired when dragging a file into the browser.
***
...
Now we need to setup an event listener for `dragenter` that will add a class to the drop target when the file enters the element. We also need an event listener for `dragleave` to remove that class when the file leaves the drop target.
...
We can access the file by taking a look at the `dataTransfer` object's `files` property. First we use an `if` statement to check that there are files associated with this drag interaction. Then we retrieve the first file and pass this to the `readTextFile` function that we will write next.
...
The final piece of the puzzle is the `readTextFile` function. This is responsible for reading the contents of a file and adding it within the `<pre>` element in your markup.
...
If you open up the [live demo]() you should be able to drop a .txt file onto the target and see the contents of the file displayed below.
...
Browser support for native drag and drop is actually pretty good. All the major desktop browsers support the API. Support amongst mobile browsers is poor though, with no mobile browsers currently supporting drag and drop.
...
The question still stands about support on mobile though. Touch screen devices form a natural environment for drag and drop interfaces and yet (despite significant advances) drag and drop in a mobile browser still feels clunky. Adding native Drag and Drop support to mobile browsers could possibly enable an experience far better than the interfaces currently being powered by JavaScript libraries.

I'm interested to hear your thoughts on the Drag and Drop API. How do you see yourself using this API in your projects? What are your thoughts about Drag and Drop on mobile?
